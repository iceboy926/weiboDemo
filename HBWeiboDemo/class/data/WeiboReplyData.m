...
-(MatchParser*)getMatch
{
    ...
    NSString *key=[NSString stringWithFormat:@"%@+%@+%@+type:%d",contact.mName,up_contact.mName,self.content,self.type];
...
-(MatchParser*)createMatch:(float)width
{
    if(_match==nil||![_match isKindOfClass:[MatchParser class]]){
        ...
        [parser match:self.content atCallBack:^BOOL(NSString * string) {
            NSString *partInStr;
            if (![atmans isKindOfClass:[NSString class]]) {
                partInStr = [atmans JSONString];
            } else {
                partInStr = (NSString*)atmans;
            }

...
-(void) showBigImageWithUrl:(NSString *)url fromView:(UIImageView *)fromView complete:(void (^)(UIView *))complete
{
    ...
    if(image1!=nil){
        ...
        UIImage *image2=[[HBHttpRequestCache shareCache] getCacheImageWithUrl:url];
        if(image2!=nil){
...
{
    UIImage* image=[[HBHttpRequestCache shareCache] getMemoryCacheImage:url];
    if(image){
        [self goBackToView:toView withImage:image];
    }else{
        UIImage *image=[[HBHttpRequestCache shareCache] getCacheImageWithUrl:url];
        if(image){
            [self goBackToView:toView withImage:image];
        }else{
